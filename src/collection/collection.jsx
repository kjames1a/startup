    ...
    fetch(`/api/collection`, {
      ...
      if (response.ok) {
        setCollection((prevCollection) =>
          prevCollection.filter((figure) => figure._id !== figureId)
        );
      }
    });
  };
  ...
  return (
    <main>
        ...
        <div className="figures">
          {collectionToDisplay.length === 0 ? (
            <p style={{ textAlign: "center", color: "gray" }}>
              No figures collected yet!
            </p>
          ) : (
            collection.slice(0, 5).map((item, index) => (
              <div key={index}>
                <img
                  src={item.figure.image}
      ...
      <div className="link-container">
        <NavLink className="fig-handdrawn-button" to="/blindboxes">
          Return to Blind Boxes
        </NavLink>
        {collection.length > 0 && (
          <button
            className="fig-handdrawn-button"
            onClick={() => handleDeleteFigure(collection[0]._id)}
          >
            Delete Oldest Figure
          </button>
        )}
      </div>
    </main>
